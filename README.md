...
🚀 Features (with emojis you asked for)

🧾 Generate Attendance — Detect faces live and mark attendance automatically.

📜 View Previous Invoices (CSV) — Persistent Attendance.csv stores recognized names & times

🔎 Search / Filter — (Easy to add) search through CSVs by name or timestamp.

💾 Save Images — Saves cropped images for recognized faces (recognized_faces/) and unknown faces (unknown_faces/).

🗑️ Delete Old Records — (Optional) delete or prune old images / CSV rows.
